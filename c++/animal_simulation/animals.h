...
template <class T> Population<T>::Population(std::list<T> new_herd, food_amount_t & food_amounts)
{
    herd = new_herd;
    foodAmounts = food_amounts;
}

template <class T> void Population<T>::evaluatePopulationBenefit()
{
    int thisBenefit;
...
{
    // Seed random number generator for "true" semi-randomness.
...
// For some set number of generations that can be input from the command line,
// generate an amount of food.
#endif
