...
# Here's the premise:
# Over 5 years ago, I was told to make a game for two people.
# This game consists of 5 switches and 5 lights. The switches are connected
# to one or more lights, meaning they turn on or off one or more of these lights.
# The goal is to turn off all of the lights.
# The first person is supposed to come up with a (working!) configuration, while 
# the second person is supposed to solve it. 
#
# This wasn't required for the assignment, but I wanted the code to be able 
# to determine if the first person's configuration was valid for the game. 
# After some thought, I determined that we could pretend that the configuration
# was a matrix representing a system of linear equations. Then if the matrix
# could be reduced to reduced row echelon form, the second person could solve
# it. This is why I included the rref code.
...
class LightGUI:
    ...
    def playModeEv(self,widget,data):
        ...
        while b < self.n:
            if self.buttonList[b].get_active() == True:
                self.lightConfiguration[b/self.k][0].append(rref.Frac(1))
            else:
                self.lightConfiguration[b/self.k][0].append(rref.Frac(0))
            self.buttonList[b].unset_flags(gtk.SENSITIVE)
            b = b + 1
        b = 0
        while b < self.k:
            self.lightConfiguration[b][1].append(rref.Frac(0))
            b = b + 1    
    ...
    def doesWork(self):
        b = 0
        rref.rref(self.lightConfiguration)
        while b < self.k:
            if self.lightConfiguration[b][0][b] != rref.Frac(1):
                return False
            b = b+1
